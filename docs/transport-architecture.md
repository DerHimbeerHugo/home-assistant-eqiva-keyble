...
Hardware testing showed that the Eqiva lock does not reliably complete the normal local BlueZ notification path. The connection reached the local BlueZ/Bleak backend, but notification activation produced GATT `0x0E` and `org.bluez.Error.NotConnected` errors.
