...
def _eqiva_on_disconnect(self: EqivaKeyBleClient, disconnected_client: Any) -> None:
    ...
    intentional_client = getattr(
        self, "_eqiva_intentional_disconnect_client", None
    )
    intentional = (
        disconnected_client is not None
        and disconnected_client is intentional_client
    )
    if intentional:
        self._eqiva_intentional_disconnect_client = None
    ...
    log_disconnect = _LOGGER.debug if intentional else _LOGGER.warning
    log_disconnect(
        "Eqiva %s disconnected (%s) during %s; last_rx=%s status_ok=%s pending=%s "
        "answer_code=%s requested_id=%s returned_id=%s; sanitized ATT trace: %s",
        self.address,
        "intentional" if intentional else "unexpected",
        stage,
        last_rx_text,
        status_ok,
        pending_text,
